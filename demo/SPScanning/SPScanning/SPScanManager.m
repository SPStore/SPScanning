...
- (void)scanWithPreView:(UIView *)preView scanRect:(CGRect)scanRect objectType:(NSArray *)objType completed:(void (^)(NSArray<SPScanResult *> *))completedBlock {
    [self configureWithPreView:preView scanRect:scanRect objectType:objType sessionPreset:nil videoGravit:nil completed:completedBlock];
}
...
- (void)scanWithPreView:(UIView *)preView
    ...
    [self configureWithPreView:preView scanRect:scanRect objectType:objType sessionPreset:sessionPreset videoGravit:videoGravit completed:completedBlock];
}
...
- (void)coverToMetadataOutputRectOfInterestForRect:(CGRect)scanRect {
    ...
    if ([_previewLayer.videoGravity isEqualToString:AVLayerVideoGravityResize]) {
        _metadataOutput.rectOfInterest = CGRectMake((scanRect.origin.y)/size.height,(size.width-(scanRect.size.width+scanRect.origin.x))/size.width, scanRect.size.height/size.height,scanRect.size.width/size.width);
        ...
        if (p1 < p2) {
            ...
            _metadataOutput.rectOfInterest = CGRectMake((scanRect.origin.y + fixPadding)/fixHeight,
                                                        (size.width-(scanRect.size.width+scanRect.origin.x))/size.width,
                                                        scanRect.size.height/fixHeight,
                                                        scanRect.size.width/size.width);
        } else {
            CGFloat fixWidth = size.height * (1/p2);
            CGFloat fixPadding = (fixWidth - size.width)/2;
            _metadataOutput.rectOfInterest = CGRectMake(scanRect.origin.y/size.height,
                                                        (size.width-(scanRect.size.width+scanRect.origin.x)+fixPadding)/fixWidth,
...
- (void)updatePreViewLayerFrame {
    ...
    // 重新设置有效扫描区域
    [self coverToMetadataOutputRectOfInterestForRect:_scanRect];
}
